...
uint8_t enemiesEaten;
uint8_t extraLivesCollected;
...
uint32_t getScore() {
	if (extraLivesCollected == 0 && score >= 10000) addExtraLife();
	if (extraLivesCollected == 1 && score >= 100000) addExtraLife();
	return score;
}
	...
	if (enemyMode != FRIGHT) {
		if (wasEaten(&myGuy, &enemy1) ||
			...
			if (lives == 0) {
				...
				dotTracker[PP2ROW] = (uint32_t) 1 << (31 - PP1COL);
			}
		}
	} else {
		//Enemies should change color and go home when eaten
		if (wasEaten(&myGuy, &enemy1) && (enemy1.color != GREEN_S || enemy1.color != playerColor[1])) {
			performRetreat(&enemy1);
		}
		if (wasEaten(&myGuy, &enemy2) && (enemy2.color != GREEN_S || enemy1.color != playerColor[2])) {
			performRetreat(&enemy2);
		}
		if (wasEaten(&myGuy, &enemy3) && (enemy3.color != GREEN_S || enemy1.color != playerColor[3])) {
			performRetreat(&enemy3);
		}
		if (wasEaten(&myGuy, &enemy4) && (enemy4.color != GREEN_S || enemy1.color != playerColor[4])) {
	...
	if (canMove(testX, testY)) {
		...
		if ((pawn == &myGuy) && isPixel(pawn->x, pawn->y)) {
			...
			if (isPowerPixel(pawn->x, pawn->y)) {
				...
				enemyMode = FRIGHT;
				enemy1.color = LAVENDAR_S;
				enemy2.color = LAVENDAR_S;
				enemy3.color = LAVENDAR_S;
				enemy4.color = LAVENDAR_S;
...
void playMatrixman(void) {
	...
	*/
	...
	while (programRunning)
	{
		...
		if (myGuy.dotCount == 244) {
			//get new level ready
			if (enemiesEaten == 16) score += 12000;
			drawScore();
			enemiesEaten = 0;
			++level;
