...
void startGame(uint8_t mode) {
	multiplayer = false;
	if (mode == 3) {
		player = Gamebox.startMultiplayer();
...
void showMenu() {
	...
	while (true) {
		uint8_t sel = Gamebox.getMovement();
		if ((sel & 5) != (old_sel & 5)) {
...
void loop(void) {
	if (multiplayer) {
		if (player == 2) {
			movement[0] = Gamebox.recvMessage() - p1_paddle;
			if (!Gamebox.sendMessage(p2_paddle + movement[1])) showMenu();
		} else {
			if (!Gamebox.sendMessage(p1_paddle + movement[0])) showMenu();
			movement[1] = Gamebox.recvMessage() - p2_paddle;
		}
	} else {
		movement[1] = 0;
		...
		if (movement[2] > 0) {
			if (difficulty == 2) {
				...
				if (p2_paddle - estimated_pos > 2) movement[1] = -3;
				else if (estimated_pos - p2_paddle > 2) movement[1] = 3;
	...
	if (multiplayer) {
		if (player == 2) {
			movement[2] = Gamebox.recvMessage() - ball[0];
			movement[3] = Gamebox.recvMessage() - ball[1];
		} else {
			if (!Gamebox.sendMessage(ball[0] + movement[2])) showMenu();
			if (!Gamebox.sendMessage(ball[1] + movement[3])) showMenu();
		}
	}
	...
	if (ball[0] <= 10 && ball[0] > 6 && movement[2] < 0) {
		if (p1_paddle+10 >= ball[1] && p1_paddle-10 <= ball[1]) {
			...
			speed = (touches >> 2) + 1;
			movement[2] = speed;
			movement[3] = (ball[1] - p1_paddle) / 5 * movement[2];
		}
		...
		if (p2_paddle+10 >= ball[1] && p2_paddle-10 <= ball[1]) {
			...
			speed = (touches >> 2) + 1;
			movement[2] = - speed;
			movement[3] = (ball[1] - p2_paddle) / 5 * -movement[2];
			hit_diff = 7 - random(15);
		}
	...
	if ((ball[0] + movement[2] <= 7 && movement[2] < 0)) {
		...
		movement[2] = min(-1, 7 - ball[0]);
		movement[3] *= movement[2];
	}
	if (ball[0] + movement[2] >= 121 && movement[2] > 0) {
		movement[3] /= movement[2];
		movement[2] = max(1, 121 - ball[0]);
		movement[3] *= movement[2];
	}
	delay(20);
	uint8_t mvmnt = Gamebox.getMovement();
